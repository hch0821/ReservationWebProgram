...
var translated = -100; //슬라이드 이미지가 왼쪽 또는 오른쪽으로 이동한 정도 (단위 : %)
var productImageIndex = 0; //슬라이드 이미지의 인덱스
...
function getDisplayInfoResponse(displayInfoId){
	...
	oReq.addEventListener("load", function() {
		...
		addProductImages(productImages, displayInfo);
		addProductContent(displayInfo);
		addEventInfo(displayInfo);
		addAverageScore(averageScore, comments);
		addComments(comments, true);
		changeAnchorUrl(".btn_review_more", "/reserv/review?displayInfoId="+displayInfoId);
		addLocationInfoArea(displayInfo, displayInfoImage);
		loadAnimation();
...
function addProductImages(productImages, displayInfo)
{
	...
	var prevProductImageButton = document.querySelector(".prev_inn > .btn_prev");
	var nextProductImageButton = document.querySelector(".nxt_inn > .btn_nxt");
...
function loadAnimation() {
	...
	parentElement.innerHTML = children[childrenLength-1].outerHTML + parentElement.innerHTML;
	children = parentElement.children;
	setTransition(children, true);
}

//슬라이드 이미지를 왼쪽 또는 오른쪽으로 넘기는 효과를 주는 함수
function animate(children, childrenLength, direction) {
	var idx = 0;
	
	setEnableArrowButton(".btn_prev", onPrevProductImageButtonClicked, false);
	setEnableArrowButton(".btn_nxt", onNextProductImageButtonClicked, false);
	...
	setTimeout(function(){
		setEnableArrowButton(".btn_prev", onPrevProductImageButtonClicked, true);
		setEnableArrowButton(".btn_nxt", onNextProductImageButtonClicked, true);
	},1000);
	
}

//이미지 슬라이드 영역에 있는 화살표 버튼을 비활성화 또는 활성화하는 함수
function setEnableArrowButton(anchorSelector, onclick, isEnabled){
	var anchor = document.querySelector(anchorSelector);
	if(isEnabled){
		anchor.onclick = onclick;
		anchor.children[0].className = anchor.children[0].className.replace(" off", "");
	}
	else{
...
//상품 내용을 뷰에 뿌리는 함수
function addProductContent(displayInfo){
...
function changeAnchorUrl(selector, url){
	document.querySelector(selector).href=url;
}

//위치 정보 탭에 해당하는 뷰의 내용들을 displayInfo 객체로부터 뿌리는 함수
function addLocationInfoArea(displayInfo, displayInfoImage){
...
//이벤트 정보를 뿌리는 함수
function addEventInfo(displayInfo){
	var productEvent;
...
function onPrevProductImageButtonClicked()
{
	...
	var swipeUlChildren = document.querySelector(".visual_img.detail_swipe").children;
	var swipeUlChildlrenLength = swipeUlChildren.length;
	animate(swipeUlChildren, swipeUlChildlrenLength, "left");
	productImageIndex--;
	if(productImageIndex < 0){
...
function onNextProductImageButtonClicked(){
	...
	var swipeUlChildren = document.querySelector(".visual_img.detail_swipe").children;
	var swipeUlChildlrenLength = swipeUlChildren.length;
	animate(swipeUlChildren, swipeUlChildlrenLength, "right");
